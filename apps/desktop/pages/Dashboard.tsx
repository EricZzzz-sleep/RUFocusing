...
import { dateLabel, duration, labels, timer } from '../src/types'
...
export default function Dashboard() {
  ...
  async function action(name: string, body: object = {}) {
    ...
    try {
      const next = await request(`/api/sessions/${name}`, body)
      setData(next); setConnected(true)
      if (next.finished) setSelected(next.finished)
  ...
  return <>
    ...
    <main>
      ...
      <div className="workspace-grid">
        ...
        <aside>
          <section className="panel observation-panel"><div className="panel-heading"><div><span className="eyebrow">THE HERE & NOW</span><h2>Observations</h2></div><span className={`camera-dot ${active && data?.observation.available ? 'on' : ''}`} aria-label={active && data?.observation.available ? 'Camera available' : 'Camera off or unavailable'} /></div>
            <div className={`observation-visual ${active && data?.observation.available && data.observation.face_count === 1 ? 'detected' : ''}`} aria-hidden="true"><div className="focus-corners"><span className="observation-glyph">◎</span></div></div>
            <h3 className="observation-title">{!active ? 'Your space, your choice' : active.status === 'break' ? 'Camera paused' : !active.camera_enabled ? 'Timer-only session' : data!.observation.available ? data!.observation.face_count === 1 ? 'Face detected' : data!.observation.face_count === 0 ? 'Looking for a face' : 'Multiple faces' : 'Camera unavailable'}</h3>
            <p className="observation-message">{active ? data!.observation.message : 'Enable the webcam when starting a session to see presence and head-pose estimates.'}</p>
            <div className="pose-readings">{(['pitch', 'yaw', 'roll'] as const).map(axis => <div key={axis}><span>{axis}</span><strong>{active && data!.observation[axis] != null ? `${data!.observation[axis]}°` : '—'}</strong></div>)}</div>
