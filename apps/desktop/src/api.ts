...
export async function jsonRequest<T>(path: string, data?: object): Promise<T> {
  ...
  if (!response.headers.get('content-type')?.includes('application/json')) throw new Error('The local service is unavailable. Restart RUFocusing. In development, use make run.')
