...
Calibration uses only the already enabled camera. Full face meshes and raw training feature samples are not persisted; training samples are discarded after validation. The feature detects landmarks, not personal identity. See [gaze tracking](gaze-tracking.md) for calibration lifetime and limitations.

Accuracy checks and reliability trials store local numerical results, rejection reasons, camera/display configuration, timestamps, and optional condition notes. Calibration failures and cancellations are retained for comparison. JSON exports are explicitly downloaded by the user and contain no frames, meshes, or model coefficients.
